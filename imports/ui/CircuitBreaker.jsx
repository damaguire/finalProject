...
export default class CircuitBreaker extends Component {
    constructor(props) {
        super(props);
        this.state = {
          result: ""
  ...
  async updateContract(event){
    const contractInstance = new web3.eth.Contract(abi, event.target.contracAddress.value);
      let response = await contractInstance.methods.stopContractActivity()
      .send({
        ...
        <Grid textAlign='center' style={{ height: '75vh', paddingLeft: "40px", paddingRight: "40px" }} verticalAlign='middle'>
          <Grid.Column style={{ maxWidth: 900 }} mobile={16} tablet={8} computer={14}>
            <Header as='h1' color='teal' textAlign='center'>
              Stop your contract from being edited!
            </Header>
            <br />
            <p style={{color: "teal"}}><b>(just in case something bad happens)</b></p>
            <Form id="updateContract" size='large' onSubmit={this.updateContract}>
              ...
              <br />
              <br />
              <Button color='teal' size='small' type="submit" form="updateContract">StopContract</Button>
              <br />
              <br />
